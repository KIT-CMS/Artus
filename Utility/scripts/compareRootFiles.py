...
def compareTree(directory1, directory2):
    ...
    for i in directory1.GetListOfKeys():
        ...
        if not obj2:
            ...
            log.debug("Object %s of type %s in %s is available in second file" % (i.GetName(), obj.IsA().GetName(), directory1.GetPath().split(":")[1]))
        identified = False

        if obj.IsA().GetName() == "TDirectoryFile":
            identified = True
            log.info("Comparing directory %s" % (i.GetName()))
            ...
            identified = True
            log.info("Comparing tree %s" % (i.GetName()))
    ...
    for n in range(nevts):
        ...
        for i in range(nleaves1):
            ...
            if (leaves1.UncheckedAt(i).GetValue() - leaves2.UncheckedAt(i).GetValue()) / value2 > 1e-6:
                log.critical("in event %d/%d different leaf value: " % (n, nevts) + str(leaves1.UncheckedAt(i).GetValue()) + ", " + str(leaves2.UncheckedAt(i).GetValue()) + " for name " + roottools.RootTools.full_leaf_name(leaves1.UncheckedAt(i)))

...
const double CrystalBall2015::pi    = TMath::Pi();
const double CrystalBall2015::SPiO2 = sqrt(TMath::Pi()/2.0);
const double CrystalBall2015::S2    = sqrt(2.0);

int RocRes2015::getBin(double x, const int NN, const double *b){
    for(int i=0; i<NN; ++i) if(x<b[i+1]) return i;
    return NN-1;
}

RocRes2015::RocRes2015():NETA(1),NTRK(1),NMIN(1){
...
int RocRes2015::getEtaBin(double feta){
    return getBin(feta,NETA,BETA);
}

int RocRes2015::getNBinDT(double v, int H){
    return getBin(v,NTRK,dtrk[H]);
}

int RocRes2015::getNBinMC(double v, int H){
    return getBin(v,NTRK,ntrk[H]);
}

void RocRes2015::dumpParams(){
    cout << NMIN << endl;
    cout << NTRK << endl;
    cout << NETA << endl;
...
void RocRes2015::init(string filename){
...
double RocRes2015::Sigma(double pt, int H, int F){
    double dpt=pt-45;
    return rmsA[H][F] + rmsB[H][F]*dpt + rmsC[H][F]*dpt*dpt;
}

double RocRes2015::kSpreadDet(double gpt, double rpt, double eta, int nlayers, double w){
...
double RocRes2015::kSmearDet(double pt, double eta, TYPE type, double v, double u){
    int H = getBin(fabs(eta), NETA, BETA);
...
double RocRes2015::kExtraDet(double pt, double eta, int nlayers, double u, double w){
    int H = getBin(fabs(eta), NETA, BETA);
    int F = nlayers-NMIN;
    double  v = ntrk[H][F]+(ntrk[H][F+1]-ntrk[H][F])*w;
    int     D = getBin(v, NTRK, dtrk[H]);
...
void RocRes2015::fillFitData(int &H, int &F, int &D, double &xmc, double &xdt, double &Rmc, double &Rdt, double pt, double eta){
...
double RocOne::kGenSmearDet(double pt, double eta, double v, double u){
    return RR.kSmearDet(pt, eta, RocRes2015::Data, v, u);
}
...
const int RoccoR2015::Nmem[RoccoR2015::Nset]={1, 100, 1, 41, 21};

RoccoR2015::RoccoR2015(){
...
RoccoR2015::RoccoR2015(std::string dirname){
    for(int s=0; s<Nset; ++s){
	for(int m=0; m<Nmem[s]; ++m){
...
double RoccoR2015::kScaleDT(int Q, double pt, double eta, double phi, TYPE T, int m){
    return RC[T][m]->kScaleDT(Q, pt, eta, phi);
}


double RoccoR2015::kScaleAndSmearMCDet(int Q, double pt, double eta, double phi, int nlayers){
    return RC[Default][0]->kScaleAndSmearMCDet(Q, pt, eta, phi, nlayers, random.Rndm(), random.Rndm());

}
	
double RoccoR2015::kScaleFromGenMCDet(int Q, double pt, double eta, double phi, double gpt, int nlayers){
    return RC[Default][0]->kScaleFromGenMCDet(Q, pt, eta, phi, gpt, nlayers, random.Rndm());
}

double RoccoR2015::kScaleAndSmearMCDet(int Q, double pt, double eta, double phi, int nlayers, double u, double w, TYPE T, int m){
    return RC[T][m]->kScaleAndSmearMCDet(Q, pt, eta, phi, nlayers, u, w);
}

double RoccoR2015::kScaleFromGenMCDet(int Q, double pt, double eta, double phi, double gpt, int nlayers, double w, TYPE T, int m){
    return RC[T][m]->kScaleFromGenMCDet(Q, pt, eta, phi, gpt, nlayers, w);
}

std::vector<std::vector<double> > RoccoR2015::kkScaleAndSmearMCDet(int Q, double pt, double eta, double phi, int nlayers){
...
std::vector<std::vector<double> > RoccoR2015::kkScaleFromGenMCDet(int Q, double pt, double eta, double phi, double gpt, int nlayers){
    //double u=random.Rndm();
    double w=random.Rndm();

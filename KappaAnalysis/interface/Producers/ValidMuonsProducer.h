...
public:
    ...
    static MuonIso ToMuonIso(std::string const &muonIso) {
        ...
        else if (muonIso_lower == "loose") return MuonIso::MEDIUM;
        ...
        else if (muonIso_lower == "tight_2016") return MuonIso::TIGHT;
        else if (muonIso_lower == "medium_2016") return MuonIso::MEDIUM;
        else if (muonIso_lower == "loose_2016") return MuonIso::LOOSE;
    ...
    ValidMuonsProducer(std::vector<KMuon *> product_type::*validMuons = &product_type::m_validMuons,
                       ...
                       std::vector <std::string> &(setting_type::*GetLowerPtCuts)(
                               void) const = &setting_type::GetMuonLowerPtCuts,
                       std::vector <std::string> &(setting_type::*GetUpperAbsEtaCuts)(
                               void) const = &setting_type::GetMuonUpperAbsEtaCuts) :
            ProducerBase<TTypes>(),
    ...
    void Init(setting_type const &settings) override {
        ...
        LambdaNtupleConsumer<TTypes>::AddFloatQuantity("leadingMuonPt",
                                                       [](event_type const &event, product_type const &product) {
                                                           return product.m_validMuons.size() >= 1
                                                                  ? product.m_validMuons[0]->p4.Pt()
                                                                  : DefaultValues::UndefinedFloat;
                                                       });
        LambdaNtupleConsumer<TTypes>::AddFloatQuantity("leadingMuonEta",
                                                       [](event_type const &event, product_type const &product) {
                                                           return product.m_validMuons.size() >= 1
                                                                  ? product.m_validMuons[0]->p4.Eta()
                                                                  : DefaultValues::UndefinedFloat;
                                                       });
        LambdaNtupleConsumer<TTypes>::AddFloatQuantity("trailingMuonPt",
                                                       [](event_type const &event, product_type const &product) {
                                                           return product.m_validMuons.size() >= 2
                                                                  ? product.m_validMuons[1]->p4.Pt()
                                                                  : DefaultValues::UndefinedFloat;
                                                       });
        LambdaNtupleConsumer<TTypes>::AddFloatQuantity("trailingMuonEta",
                                                       [](event_type const &event, product_type const &product) {
                                                           return product.m_validMuons.size() >= 2
                                                                  ? product.m_validMuons[1]->p4.Eta()
                                                                  : DefaultValues::UndefinedFloat;
                                                       });
        LambdaNtupleConsumer<TTypes>::AddFloatQuantity("leadingMuMinusPt",
                                                       [](event_type const &event, product_type const &product) {
                                                           for (unsigned int i = 0;
                                                                i < product.m_validMuons.size(); ++i) {
        ...
        LambdaNtupleConsumer<TTypes>::AddFloatQuantity("leadingMuPlusPt",
                                                       [](event_type const &event, product_type const &product) {
                                                           for (unsigned int i = 0;
                                                                i < product.m_validMuons.size(); ++i) {
                                                               if (product.m_validMuons[i]->charge() > 0) {
        ...
        LambdaNtupleConsumer<TTypes>::AddFloatQuantity("leadingMuMinusEta",
                                                       [](event_type const &event, product_type const &product) {
                                                           for (unsigned int i = 0;
                                                                i < product.m_validMuons.size(); ++i) {
                                                               if (product.m_validMuons[i]->charge() < 0) {
        ...
        LambdaNtupleConsumer<TTypes>::AddFloatQuantity("leadingMuPlusEta",
                                                       [](event_type const &event, product_type const &product) {
                                                           for (unsigned int i = 0;
                                                                i < product.m_validMuons.size(); ++i) {
                                                               if (product.m_validMuons[i]->charge() > 0) {
    ...
    void Produce(event_type const &event, product_type &product,
        ...
        if ((validMuonsInput == ValidMuonsInput::AUTO && (product.m_correctedMuons.size() > 0)) ||
            (validMuonsInput == ValidMuonsInput::CORRECTED)) {
            LOG(DEBUG) << "Using corrected Muons as input; Number of m_correctedMuons: "
                       << product.m_correctedMuons.size();
            ...
            for (std::vector < std::shared_ptr < KMuon > > ::iterator muon = product.m_correctedMuons.begin();
                    muon != product.m_correctedMuons.end();
            ++muon)
            {
            ...
            LOG(DEBUG) << "Standard input (uncorrected); Number of m_muons: " << event.m_muons->size();
        ...
        if (muonIso == MuonIso::TIGHT) {
            LOG(DEBUG) << "--TIGHT";
        } else if (muonIso == MuonIso::MEDIUM) {
            LOG(DEBUG) << "--MEDIUM";
        ...
        for (std::vector<KMuon *>::iterator muon = muons.begin(); muon != muons.end(); ++muon) {
            bool validMuon = true;
            LOG(DEBUG) << "Muon:";
            LOG(DEBUG) << "Muon pt: " << (*muon)->p4.Pt() << " charge: " << (*muon)->charge() << " eta: "
                       << (*muon)->p4.Eta() << " phi: " << (*muon)->p4.Phi();
            ...
            if (muonID == MuonID::NONE) {
                ...
                if (settings.GetYear() >= 2015 && settings.GetYear() <= 2018) {
                    validMuon = validMuon && IsTightMuonRun2(*muon, event, product);
                ...
                if (settings.GetYear() >= 2015 && settings.GetYear() <= 2018) {
                    validMuon = validMuon && IsMediumMuonRun2(*muon, event, product);
                } else {
                    LOG(FATAL) << "Medium MuonID for year " << settings.GetYear() << " not yet implemented!";
                }
            } else if (muonID == MuonID::LOOSE) {
                if (settings.GetYear() >= 2015 && settings.GetYear() <= 2018) {
                    validMuon = validMuon && IsLooseMuonRun2(*muon, event, product);
                } else if (settings.GetYear() == 2012) {
            ...
            if (muonIsoType == MuonIsoType::NONE) {
                ...
                if (settings.GetYear() < 2015) {
                    if (muonIso == MuonIso::TIGHT) {
                        validMuon = validMuon &&
                                    ((((*muon)->pfIso() / (*muon)->p4.Pt()) < 0.12f) ? settings.GetDirectIso()
                                                                                     : (!settings.GetDirectIso()));
                    } else if (muonIso == MuonIso::LOOSE) {
                        validMuon = validMuon &&
                                    ((((*muon)->pfIso() / (*muon)->p4.Pt()) < 0.20f) ? settings.GetDirectIso()
                                                                                     : (!settings.GetDirectIso()));
                ...
                // https://twiki.cern.ch/twiki/bin/viewauth/CMS/SWGuideMuonIdRun2
                    ...
                    if (muonIso == MuonIso::TIGHT) {
                        LOG(DEBUG) << "Iso Value: " << ((*muon)->pfIsoR04() / (*muon)->p4.Pt())
                                   << " < 0.15 for TIGHT iso?";
                        validMuon = validMuon &&
                                    ((((*muon)->pfIsoR04() / (*muon)->p4.Pt()) < 0.15f) ? settings.GetDirectIso()
                                                                                        : (!settings.GetDirectIso()));
                        LOG(DEBUG) << "Valid muon: " << validMuon;
                    } else if (muonIso == MuonIso::MEDIUM) {
                        LOG(DEBUG) << "Iso Value: " << ((*muon)->pfIsoR04(0.5) / (*muon)->p4.Pt())
                                   << " < 0.2 for MEDIUM iso?";
                        validMuon = validMuon &&
                                    ((((*muon)->pfIsoR04(0.5) / (*muon)->p4.Pt()) < 0.20f) ? settings.GetDirectIso()
                                                                                           : (!settings.GetDirectIso()));
                        LOG(DEBUG) << "Valid muon: " << validMuon;
                    } else if (muonIso == MuonIso::LOOSE) {
                        LOG(DEBUG) << "Iso Value: " << ((*muon)->pfIsoR04() / (*muon)->p4.Pt())
                                   << " < 0.25 for LOOSE iso?";
                        validMuon = validMuon &&
                                    ((((*muon)->pfIsoR04() / (*muon)->p4.Pt()) < 0.25f) ? settings.GetDirectIso()
                                                                                        : (!settings.GetDirectIso()));
                        LOG(DEBUG) << "Valid muon: " << validMuon;
                    } else if (muonIso == MuonIso::TIGHT_2015) {
                        LOG(DEBUG) << "Iso Value: " << ((*muon)->pfIsoR04(0.5) / (*muon)->p4.Pt())
                                   << " < 0.15 for TIGHT_2015 iso?";
                        validMuon = validMuon &&
                                    ((((*muon)->pfIsoR04(0.5) / (*muon)->p4.Pt()) < 0.15f) ? settings.GetDirectIso()
                                                                                           : (!settings.GetDirectIso()));
                        LOG(DEBUG) << "Valid muon: " << validMuon;
                    } else if (muonIso == MuonIso::LOOSE_2015) {
                        LOG(DEBUG) << "Iso Value: " << ((*muon)->pfIsoR04(0.5) / (*muon)->p4.Pt())
                                   << " < 0.3 for LOOSE_2015 iso?";
                        validMuon = validMuon &&
                                    ((((*muon)->pfIsoR04(0.5) / (*muon)->p4.Pt()) < 0.30f) ? settings.GetDirectIso()
                                                                                           : (!settings.GetDirectIso()));
                        LOG(DEBUG) << "Valid muon: " << validMuon;
                    } else if (muonIso == MuonIso::FAKEABLE) {
                ...
                if (muonIso == MuonIso::TIGHT) {
                    LOG(DEBUG) << "Iso Value: " << ((*muon)->trackIso / (*muon)->p4.Pt())
                               << " < 0.05 for TIGHT detector iso?";
                    validMuon = validMuon && ((((*muon)->trackIso / (*muon)->p4.Pt()) < 0.05f) ? settings.GetDirectIso()
                                                                                               : (!settings.GetDirectIso()));
                    LOG(DEBUG) << "Valid muon: " << validMuon;
                } else if (muonIso == MuonIso::LOOSE) {
                    LOG(DEBUG) << "Iso Value: " << ((*muon)->trackIso / (*muon)->p4.Pt())
                               << " < 0.10 for LOOSE detector iso?";
                    validMuon = validMuon && ((((*muon)->trackIso / (*muon)->p4.Pt()) < 0.10f) ? settings.GetDirectIso()
                                                                                               : (!settings.GetDirectIso()));
                    LOG(DEBUG) << "Valid muon: " << validMuon;
                } else if (muonIso != MuonIso::NONE) {
                    LOG(FATAL) << "Muon isolation of type " << Utility::ToUnderlyingValue(muonIso)
                ...
                LOG(FATAL) << "Muon isolation type of type " << Utility::ToUnderlyingValue(muonIsoType)
                           << " not yet implemented!";
            }
        ...
        LOG(DEBUG) << "Number of m_validMuonsMember: " << (product.*m_validMuonsMember).size();
        LOG(DEBUG) << "Number of m_invalidMuonsMember: " << (product.*m_invalidMuonsMember).size();
...
private:
    ...
    bool IsLooseMuonRun2(KMuon *muon, event_type const &event, product_type &product) const {
        return muon->idLoose();
    }

    // https://twiki.cern.ch/twiki/bin/viewauth/CMS/SWGuideMuonIdRun2#Medium_Muon
    bool IsMediumMuonRun2(KMuon *muon, event_type const &event, product_type &product) const {
        return muon->idMedium();
    }

    // https://twiki.cern.ch/twiki/bin/viewauth/CMS/SWGuideMuonIdRun2#Tight_Muon
    bool IsTightMuonRun2(KMuon *muon, event_type const &event, product_type &product) const {
        return muon->idTight();
    }
    ...
    bool
    IsFakeableMuonIso(KMuon *muon, event_type const &event, product_type &product, setting_type const &settings) const {
        bool validMuon = true;
        ...
        if (muon->p4.Pt() <= 20.0) {
            ...
            validMuon = validMuon &&
                        (((muon->trackIso / muon->p4.Pt()) < 0.4f) ? settings.GetDirectIso() :
                         (!settings.GetDirectIso()) &&
                         ((muon->ecalIso / muon->p4.Pt()) < 0.4f) ? settings.GetDirectIso() :
                         (!settings.GetDirectIso()) &&
                         ((muon->hcalIso / muon->p4.Pt()) < 0.4f) ? settings.GetDirectIso()
                                                                  : (!settings.GetDirectIso()));
        }
